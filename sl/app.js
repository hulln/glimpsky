        ...
        function renderVisualizations(rows, mode) {
            ...
            const activityLabel = mode === 'likes' ? 'všečki' : 'objave';
            ...
            const timelineScopeBase = hasDateRangeFilter
                ? 'Datumski filtri vplivajo na graf in seznam spodaj.'
                : `Prikaz ${activeRange.scopeLabel}${snapshot.timeline.isComplete ? '.' : ' (starejši so lahko še nenaloženi).'}`;
            const timelineGroupingNote = snapshot.timeline.bucketMode === 'week'
                ? ' Zaradi berljivosti je časovnica združena po tednih.'
                : (snapshot.timeline.bucketMode === 'month' ? ' Zaradi berljivosti je časovnica združena po mesecih.' : '');
            const timelineScopeNote = `${timelineScopeBase}${timelineGroupingNote}`;
            const analyticsOnlyNote = hasDateRangeFilter
                ? ''
                : '<p class="viz-activity-note">Gumbi Teden, Mesec in Leto spreminjajo samo graf. Za filtriranje seznama uporabite datumske filtre zgoraj.</p>';
            ...
            const activeKpiLabel = snapshot.timeline.bucketMode === 'week'
                ? 'aktivni tedni'
                : (snapshot.timeline.bucketMode === 'month' ? 'aktivni meseci' : 'aktivni dnevi');
            const peakKpiLabel = snapshot.timeline.bucketMode === 'week'
                ? 'vrh/teden'
                : (snapshot.timeline.bucketMode === 'month' ? 'vrh/mesec' : 'vrh/dan');
            ...
            const likesSeriesToggle = mode === 'likes'
                ? `
                    <label class="viz-line-mode-label" for="vizLikesMainOnlyInput">
                        ...
                        <span>Prikaži samo črto Vsi všečki</span>
                    </label>
                `
                : '';
            ...
            const mixRows = snapshot.mix.length > 0
                ...
                : '<div class="viz-empty">Ni podatkov v izbranem razponu.</div>';
            ...
            const topRows = snapshot.topAuthors.length > 0
                ? snapshot.topAuthors.map((entry, idx) => {
                    ...
                    return `
                        <li>
                            <button class="viz-top-btn" type="button" data-author-query="${escapeHtml(query)}" title="Filtriraj po avtorju">
                ...
                : `<li class="viz-empty">${mode === 'likes' ? 'Ni najdenih avtorjev z všečki.' : 'Ni najdenih ciljnih interakcij.'}</li>`;
            const hasAuthorFilter = Boolean(elements.searchAuthor.value.trim());
            const resetAuthorControl = hasAuthorFilter
                ? `<button class="viz-reset-btn" type="button" id="vizResetAuthorBtn">Ponastavi avtorja</button>`
                : '';
            ...
            elements.visualizations.innerHTML = `
                <div class="viz-card viz-card-wide">
                    <div class="viz-head">
                        <div>
                            <h4>Časovnica aktivnosti</h4>
                            <p>${timelineTitle}</p>
                            <p class="viz-activity-note">${timelineScopeNote}</p>
                            ${analyticsOnlyNote}
                            ${timelineNote}
                            <div class="viz-range-controls" role="group" aria-label="Razpon časovnice">
                                ${rangeControls}
                            </div>
                            ${likesSeriesToggle}
                        </div>
                ...
                <div class="viz-card">
                    <div class="viz-head viz-head-tight">
                        <div>
                            <h4>Mešanica vsebine</h4>
                            <p>Trenutno izbran analitični razpon</p>
                        </div>
                    </div>
                ...
                <div class="viz-card">
                    <div class="viz-head viz-head-tight">
                        <div>
                            <h4>${mode === 'likes' ? 'Najpogosteje všečkani avtorji' : 'Najpogostejši cilji interakcij'}</h4>
                            <p>Klik za filter po avtorju</p>
                        </div>
                        ${resetAuthorControl}
                    </div>
        ...
        function buildTimelineBuckets(rows, mode, rangeKey = ANALYTICS_RANGE_DEFAULT, windowDays = 42) {
            ...
            if (!firstActivePoint) {
                activityNote = isComplete
                    ? 'V tem datumskem razponu ni ujemajoče aktivnosti.'
                    : 'V tem datumskem razponu med trenutno naloženimi rezultati ni ujemajoče aktivnosti.';
            }
        ...
        function buildTimelineSvg(points, seriesDefs) {
            ...
            const defs = Array.isArray(seriesDefs) && seriesDefs.length > 0
                ? seriesDefs
                : [{ key: 'total', label: 'Skupaj', lineColor: '#3b82f6' }];
            const width = 640;
            const height = 208;
            const topPad = 12;
            const sidePad = 12;
            ...
            return `
                <svg class="viz-chart-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="Časovnica aktivnosti">
                    ${grid}
                    ${seriesHtml}
                    ${hitsHtml}
                </svg>
        ...
        function attachTimelineInteractions(mode, seriesDefs = []) {
            ...
            const unitLabel = mode === 'likes' ? 'všečkov' : 'objav';
        ...
        function getAnalyticsRangeDefinitions() {
            return [
                { key: '7d', label: 'Teden', scopeLabel: 'zadnjega tedna', ariaLabel: 'Prikaži zadnji teden', days: 7 },
                { key: '30d', label: 'Mesec', scopeLabel: 'zadnjega meseca', ariaLabel: 'Prikaži zadnji mesec', days: 30 },
                { key: '365d', label: 'Leto', scopeLabel: 'zadnjega leta', ariaLabel: 'Prikaži zadnje leto', days: 365 },
                { key: 'custom', label: 'Po meri', scopeLabel: 'razpona po meri', ariaLabel: 'Uporabi datumski razpon po meri', days: null }
            ];
        }
        ...
        function getAnalyticsTargetStartDate(rangeKey) {
            if (rangeKey === 'custom') {
                ...
                // Samo datum-do nima spodnje meje, zato je za popolno pokritost potrebna celotna zgodovina.
                return null;
            }
        ...
        function getVisualizationTypeDefs(mode) {
            if (mode === 'likes') {
                return [
                    { key: 'text', label: 'Samo besedilo', colorClass: 'viz-fill-text', lineColor: '#3b82f6' },
                    { key: 'links', label: 'Povezave', colorClass: 'viz-fill-links', lineColor: '#6366f1' },
                    { key: 'media', label: 'Mediji', colorClass: 'viz-fill-media', lineColor: '#ef4444' },
                    { key: 'replies', label: 'Odgovori', colorClass: 'viz-fill-replies', lineColor: '#0ea5e9' },
                    { key: 'quotes', label: 'Citati', colorClass: 'viz-fill-quotes', lineColor: '#f59e0b' }
                ];
            }
            return [
                { key: 'originals', label: 'Izvirne', colorClass: 'viz-fill-originals', lineColor: '#3b82f6' },
                { key: 'replies', label: 'Odgovori', colorClass: 'viz-fill-replies', lineColor: '#0ea5e9' },
                { key: 'reposts', label: 'Poobjave', colorClass: 'viz-fill-reposts', lineColor: '#14b8a6' },
                { key: 'quotes', label: 'Citati', colorClass: 'viz-fill-quotes', lineColor: '#f59e0b' },
                { key: 'links', label: 'Povezave', colorClass: 'viz-fill-links', lineColor: '#6366f1' }
            ];
        }
        ...
        function getTimelineSeriesDefs(mode) {
            ...
            return [
                { key: 'allLikes', label: 'Vsi všečki', lineColor: '#14b8a6', lineWidth: 1.8 },
                ...defs
            ];
        }
        ...
        function updateCoverageHint() {
            ...
            const isComplete = allPostsLoaded || !currentCursor;
            const unitLabel = currentMode === 'likes' ? 'všečkov' : 'elementov';
            ...
            if (isComplete || rangeCovered) {
                elements.coverageHint.innerHTML = `
                    <span class="coverage-text is-complete">${isComplete
                        ? 'Za trenutno analitiko in filtre je naložen popoln niz.'
                        : 'Naložen niz v celoti pokriva izbrani časovni razpon.'}</span>
                `;
            ...
            elements.coverageHint.innerHTML = `
                <span class="coverage-text is-partial">Trenutno je naloženih ${loadedCount} ${unitLabel}. Za analitiko in filtre lahko manjkajo starejši rezultati. Uporabite krajši razpon, Naloži več ali Naloži vse podatke.</span>
                <button class="coverage-load-btn" type="button" id="coverageLoadAllBtn">Naloži vse podatke</button>
            `;
            elements.coverageHint.classList.add('show');
            setAnalyticsExpanded(analyticsExpanded);
